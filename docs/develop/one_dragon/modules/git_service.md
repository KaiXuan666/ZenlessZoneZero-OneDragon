...
本地模块路径为 `one_dragon.envs.git.git_service`。合并上游更新时，启动器、开发入口和 GUI 都使用该路径及上游新增的 `GitSyncStatus`。

已有 remote 的 URL、push URL 和 SSH 配置保持不变；查询其他代码源时使用匿名 remote，不把候选源写入正式仓库。只有没有 remote 时才创建项目主仓库地址。仓库损坏时，能够读取到自定义 Fork/SSH origin 的仓库不自动重建，以免丢失用户配置；旧仓库无法打开时仍按上游流程保留完整 Git 目录备份。

网络 fetch 优先在临时 bare 仓库调用系统 Git，沿用上游生成的分支/tag refspec、浅克隆深度和对象复用配置。系统 Git 使用原子引用更新且不自动跟随 tag；最长执行 60 秒，期间向主流程报告存活状态，因此 CLI 阶段受总时限约束，不以输出静默判断超时。操作作废时终止该 Git 进程，不再导入结果。系统 Git 缺失、失败或超时后使用原有 libgit2 兜底，保留候选代码源回退、临时仓库导入和浅边界同步逻辑。

版本标签查询对每个候选源先运行系统 Git `ls-remote --tags`（30 秒超时），失败才通过匿名 remote 调用 libgit2。两个命令均禁用交互式凭据提示，Windows 下隐藏命令窗口；不修改用户的 SSH 配置。系统 Git 正常工作时，网络请求不进入可能触发本地闪退的 libgit2 路径；失败兜底仍保留其原生调用风险。

对应回归测试位于 `zzz-od-test/test/one_dragon/envs/test_git_upstream_merge.py`，使用临时本地仓库验证远端保护、真实 fetch 与标签查询、失败回退和取消流程，不启动游戏或更新当前工作区。
