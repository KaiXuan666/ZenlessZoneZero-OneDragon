...
def _fetch_with_system_git(
    ...
    """优先在临时仓库执行系统 Git，失败后允许原有 libgit2 流程兜底。"""
    command = ['git', 'fetch', '--atomic', '--no-tags', '--update-shallow']
    if depth > 0:
        command.append(f'--depth={depth}')
    command.extend(['origin', *refspecs])
    env = os.environ.copy()
    ...
    try:
        with subprocess.Popen(
            ...
            try:
                while True:
                    if abandoned.is_set():
                        raise TimeoutError('本次 Git fetch 已作废')
                    if time.monotonic() - started_at >= 60:
                        raise TimeoutError('系统 Git fetch 超过 60 秒')
                    progress_callback(0.0, '系统 Git 正在拉取代码...')
                    try:
                    ...
                    if process.returncode == 0:
                        return True
                    log.warning(f'系统 Git fetch 失败，尝试 libgit2 兜底: {stderr.strip()}')
                    return False
            finally:
        ...
        if abandoned.is_set():
            raise
        log.warning('系统 Git 不可用或拉取超时，尝试 libgit2 兜底', exc_info=True)
        return False
...
def _fetch_remote_worker(
    ...
    try:
        ...
        try:
            ...
            if not _fetch_with_system_git(
                temp_repo_dir, refspecs, actual_depth, proxy, report_progress, abandoned,
            ):
            ...
            if abandoned.is_set() or not can_fallback:
                raise
            ...
            callbacks = _FetchProgressRemoteCallbacks(report_progress)
            if not _fetch_with_system_git(
                temp_repo_dir, [target_refspec], 1, proxy, report_progress, abandoned,
            ):
                with _temporary_fetch_timeout_context():
                    remote.fetch(
                        refspecs=[target_refspec],
...
class GitService:
    ...
    def _ensure_remote(self, remote_url: str | None = None) -> Remote:
        """保留现有远端配置，候选代码源使用不落盘的匿名 remote。"""
        repo = self._open_repo()
        remote_name = self.env_config.git_remote
        if remote_name in repo.remotes.names():
            remote = repo.remotes[remote_name]
            if remote_url is None or remote.url == remote_url:
                return remote
            return repo.remotes.create_anonymous(remote_url)
        ...
        log.info(f'创建远程仓库: {remote_name} -> {primary_url}')
        repo.remotes.create(remote_name, primary_url)
        if remote_url is not None and remote_url != primary_url:
            return repo.remotes.create_anonymous(remote_url)
    ...
    def _restore_origin(self) -> bool:
        """保留已有 Fork/SSH 地址，仅在缺少 remote 时配置项目主仓库。"""
        primary_url = self._get_repository_url(self.repo_config.primary_repository, use_gh_proxy=False)
        if not primary_url:
            return False
        try:
        ...
        try:
            ...
            try:
                ...
                if extra_remotes:
                    ...
                    return GitSyncStatus.LOCAL_UPDATE_FAILED, failure_message
                if 'origin' in opened_repo.remotes.names():
                    origin_url = opened_repo.remotes['origin'].url
                    if self._find_repository(origin_url) is None:
                        log.error('检测到自定义远端地址，暂不自动重建，以保留 Fork/SSH 配置')
                        return GitSyncStatus.LOCAL_UPDATE_FAILED, failure_message
            ...
            for repository, repository_url in self._get_repository_candidates():
                ...
                try:
                    ...
                    try:
                        ...
                        log.warning('系统 Git 标签查询失败，尝试 libgit2 兜底', exc_info=True)
                        ...
                        with self._temporary_fetch_timeout():
                            heads = [
                                head.name
                                for head in remote.list_heads(callbacks=callbacks, proxy=proxy)
                            ]
